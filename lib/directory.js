...
internals.schema = Joi.object({
    ...
    index: Joi.alternatives(Joi.boolean(), Joi.string().regex(/^\w*\.?\w*$/)),
    listing: Joi.boolean(),
    ...
    var handler = function (request, reply) {
        ...
        Items.serial(paths, function (path, next) {
            ...
            File.load(path, request, { lookupCompressed: settings.lookupCompressed }, function (response) {
                ...
                // Directory
                var indexName = (settings.index === void 0 || settings.index === true) ? 'index.html' : settings.index;    // Defaults to 'index.html'
                if (!indexName &&
                    !settings.listing) {
