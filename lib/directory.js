...
internals.generateListing = function (path, resource, selection, hasTrailingSlash, settings, request, reply) {
    ...
    Fs.readdir(path, (err, files) => {
        ...
        const context = {
            location: resource,
            parent: selection ? internals.pathEncode(resource.substring(0, resource.lastIndexOf('/', resource.length - (hasTrailingSlash ? 2 : 1)))) + '/' : null,
            files: []
        };
        ...
        for (let i = 0; i < files.length; ++i) {
            ...
            if (settings.showHidden || !isHidden) {
                context.files.push({
                    uri: internals.pathEncode(prefix + files[i]),
                    name: files[i],
                    hidden: isHidden
                });
            }
