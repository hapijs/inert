...
internals.renderSimpleListing = function (context, callback) {

    const display = Hoek.escapeHtml(context.resource);
    let html = '<html><head><title>' + display + '</title></head><body><h1>Directory: ' + display + '</h1><ul>';

    if (context.parentPath) {
        html = html + '<li><a href="' + internals.pathEncode(context.parentPath) + '">Parent Directory</a></li>';
    }

    for (let i = 0; i < context.files.length; ++i) {
        const file = context.files[i];
        html = html + '<li><a href="' + internals.pathEncode(file.path) + '">' + Hoek.escapeHtml(file.name) + '</a></li>';
    }
...
internals.generateListing = function (path, resource, selection, hasTrailingSlash, settings, request, reply) {
    ...
    Fs.readdir(path, (err, files) => {
        ...
        const context = {
            resource,
            parentPath: selection ? resource.substring(0, resource.lastIndexOf('/', resource.length - (hasTrailingSlash ? 2 : 1))) + '/' : null,
            files: []
        };

        const prefix = resource + (selection && !hasTrailingSlash ? '/' : '');
        ...
        renderFn(context, (err, rendered, type, charset) => {

            if (err) {
                return reply(Boom.wrap(err, 500, 'Failed to render file listing'));
            }
