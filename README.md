...
methods for serving static files and directories, as well as decorating the [responder](https://github.com/hapijs/hapi/blob/master/API.md#responder-interface)
...
## Index
  ...
  - [Usage](#usage)
      ...
      - [`responder.file(path, [options])`](#responderfilepath-options)
...
If you need more control, the `responder.file()` method is available to use inside handlers:
...
server.route({
    ...
    handler(request, responder) {
        ...
        return responder.file(path).vary('x-magic');
    }
});
...
server.ext('onPreResponse', (request, responder) => {
    ...
    if (response.isBoom &&
        ...
        return responder.file('404.html').code(404);
    }
